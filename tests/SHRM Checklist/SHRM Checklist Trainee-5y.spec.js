...
test("hubspot login", async ({ browser }) => {
  ...
  await page.goto('https://app.hubspot.com/contacts/4209490/lists/21348/filters');
  ...
  await page.waitForTimeout(3000);
  await page.frameLocator('internal:role=dialog >> iframe').getByPlaceholder('Search').fill('SHRM Checklist 5 years');
  await page.waitForTimeout(3000);
  await page.frameLocator('internal:role=dialog >> iframe').locator('a').filter({ hasText: 'Checklist of Requirements SHRM - Trainee 5 years' }).click();
  await page.waitForTimeout(5000);
  ...
  //await page2.frameLocator('#kSolas-editor-iframe').getByRole('button', { name: 'Send document' }).click();
