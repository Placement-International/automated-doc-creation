...
test("hubspot login", async ({ browser }) => {
  ...
  await page.goto('https://app.hubspot.com/contacts/4209490/lists/20666/filters');
  ...
  await page.waitForTimeout(3000);
  await page.frameLocator('internal:role=dialog >> iframe').getByPlaceholder('Search').fill('CIEE Checklist Intern');
  await page.waitForTimeout(3000);
  await page.frameLocator('internal:role=dialog >> iframe').locator('a').filter({ hasText: 'Checklist of Requirements CIEE - Intern' }).click();
  await page.waitForTimeout(5000);
