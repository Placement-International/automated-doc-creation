...
test("hubspot login", async ({ browser }) => {
  ...
  await page.goto('https://app.hubspot.com/contacts/4209490/lists/21339/filters');
  ...
  await page.waitForTimeout(3000);
  await page.frameLocator('internal:role=dialog >> iframe').getByPlaceholder('Search').fill('CIEE Checklist 1 year');
  await page.waitForTimeout(3000);
  await page.frameLocator('internal:role=dialog >> iframe').locator('a').filter({ hasText: 'Checklist of Requirements CIEE - Trainee 1 year' }).click();
  await page.waitForTimeout(5000);
